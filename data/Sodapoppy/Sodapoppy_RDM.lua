-- Original: Motenten / Modified: Arislan / Now: Mekaider
...
function user_setup()
    ...
    state.WeaponSet = M{['description']='Weapon Set', 'CroceaMors', 'Naegling', 'Idle'}
    ...
    send_command('bind f9 gs c toggle WeaponLock; gs c cycle WeaponSet; gs c toggle WeaponLock;')
...
function init_gear_sets()
    ...
    sets.Moving = {right_ring="Shneddick Ring"} -- {legs="Carmine Cuisses +1"}
	...
	-- sets.AeolianEdge = {main="Tauret", sub="Thibron"}
    sets.Idle = {main="Mpaca's Staff", sub="Enki Strap"}
...
function check_enspell()
    if buffactive['enfire'] or buffactive['enthunder'] or buffactive['enwater'] or buffactive['enaero'] or buffactive['enstone'] or buffactive['enblizzard'] then
        state.EnspellMode:set('true')
    else
        state.EnspellMode:set('false')
    end
end
